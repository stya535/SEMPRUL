...
acil = SADISBOT.LINE() 
acil.login(token="Er9fcGlgxaZ9mWH69tB0.9q2iIcfJltjghJCvoSD1Wa.Z7BdcKteVSsjMAjsLuXmJUXFeLbMAjxu4OL/Qf74Ad4=")
acil.loginResult()

pb1 = SADISBOT.LINE() 
pb1.login(token="ErWGMcso22iCPvi7Y8fa.8FCRsDJUEScJ1omJxCYckG.jp99s/fvpageuEUlGHinrLsSrHp7vyqwsPXDwrumYZQ=")
pb1.loginResult()

pb2 = SADISBOT.LINE() 
pb2.login(token="ErLOCoVJoKKW0ApQSVEc.Ogm/dkZS2yUpS5jmWiBjda.rA8RIWLdZVp2UDDsI6SuFvSmUGFdvG8MoWX9lsnDmZM=")
pb2.loginResult()
...
admsa = "uf50d888821632d32461e37153ac775c0"
...
wait = {
    ...
    'timeline':False,
    'autoAdd':True,
    'message':"""❂••••AUTO ADD BY BOTS SADIS••••❂\n\n❂•SALAM SANTUN❂""",
    "lang":"JP",
    "comment1":"❂••••AUTO ADD BY BOTS SADIS••••❂\n\n❂•SALAM SANTUN❂",
...
def bot(op):
    try:
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                msg.to = msg.from_
                if msg.from_ == "uf50d888821632d32461e37153ac775c0":
                    if "join:" in msg.text:
...
#                else:
                ...
                if len(string.decode('utf-8')) <= 20:
                    ...
                    acil.sendText(msg.to,"􀜁􀇔􏿿semua nama Botz telah di update menjadi\n👉 " + string + "👈")
...
#--------------------------------------------------------
                ...
                acil.sendMessage(msg)
                acil.sendText(msg.to,"❂•••••{CREATOR BOTS SADIS}•••••❂")
                ...
                for i in gid:
                    ...
                    ki6.leaveGroup(i)
                if wait["lang"] == "JP":
                    acil.sendText(msg.to,"BOTS SADIS Sudah Keluar Di semua grup")
                else:
...
#---------------------------------------------- 
#---------------------- = NUKE = ------------------
            elif "Ayoo" in msg.text:
                if msg.toType == 2:
                    print "Ayoo ok"
                    _name = msg.text.replace("Ayoo","")
                    gs = acil.getGroup(msg.to)
                    gs = pb1.getGroup(msg.to)
                    gs = pb2.getGroup(msg.to)
                    start = time.time()
                    pb1.sendText(msg.to, "Bahaha ")
                    elapsed_time = time.time() - start
                    pb1.sendText(msg.to, "%sseconds" % (elapsed_time))
                    pb2.sendText(msg.to, "Santay yee")
                    acil.sendText(msg.to, "Iyaa boss")
                    acil.sendText(msg.to,"Wong yen lagi anu..anane gor anuuu ae nananaan")
                    targets = []
                    for g in gs.members:
                        ...
                        for target in targets:
                          if not target in Bots:
                            try:
                                ...
                                acil.sendText(msg,to,"Pas Finish")
                                pb2.sendText(msg,to,"Pas Succes Boss")
#-------------------- = NUKE FINISH = ----------------------------- 
...
#-----------------------------------------------
#-----------------------------------------------
            elif msg.text.lower() == '333':
                if msg.toType == 2:
                    ginfo = acil.getGroup(msg.to)
                    try:
                        acil.sendText(msg.to,"􀜁􀇔􏿿Bye SALAM SANTUN YEE\nSampe ketemu lagi\nBOTS SADIS.."  +  str(ginfo.name)  + "")
...
#-----------------------------------------------
                ...
                ginfo = acil.getGroup(msg.to)
                acil.sendText(msg.to,"Welcome to " + str(ginfo.name))
